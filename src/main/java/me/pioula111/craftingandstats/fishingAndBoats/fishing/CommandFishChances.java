...
public class CommandFishChances implements CommandExecutor {
    private LootManager lootManager;

    public CommandFishChances(LootManager lootManager) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length != 2) {
            sender.sendMessage(ChatColor.RED + "Zła komenda! Użyj /fishchances <id> <szanse_w_procentach>");
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage(ChatColor.RED + "Zły format id lub procentów poprawna to: /fishchances 1 23.5");
            return true;
        }

        id--;
        if (id < 0 || id >= items.size()) {
            sender.sendMessage(ChatColor.RED + "Niepoprawne id, wpisz /fishchances, by zobaczyć id");
            return true;
        }
